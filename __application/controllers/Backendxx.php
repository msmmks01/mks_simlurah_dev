	...
	function get_form($mod)
	{
		...
		switch ($mod) {
			...
			case "laporan_hasil_kegiatan":
				...
				$this->nsmarty->assign(
					...
					$this->lib->fillcombo(
						"perihal_hasil_agenda",
						"return",
						($sts == "edit" ? $data['perihal_hasil_agenda'] : "")
					)
				);
				break;
			...
			case "verifikasi_lpj_rt_rw":
				...
				$rt_rw_id = '';
				if ($this->input->post('tbl_data_rt_rw_id')) {
					$rt_rw_id = $this->input->post('tbl_data_rt_rw_id');
				} elseif ($this->input->get('tbl_data_rt_rw_id')) {
					$rt_rw_id = $this->input->get('tbl_data_rt_rw_id');
				} elseif ($this->input->post('id')) {
					$rt_rw_id = $this->input->post('id');
	...
	function cetak($mod, $p1 = "", $p2 = "", $p3 = "", $p4 = "")
	{
		...
		switch ($mod) {
			...
			case "laporan_faskes":
				...
				$this->nsmarty->assign("setting", $this->setting);
				$this->nsmarty->assign("tgl_cetak", $tgl_cetak);
				$data = $this->mbackend->getdata('laporan_faskes', 'result_array');

				$filename = "laporan-faskes-" . date('YmdHis');

				$temp = "backend/cetak/laporan_faskes.html";
			...
			case "laporan_hasil_kegiatan":
				...
				$filename = "laporan-hasil-kegiatan-" . date('YmdHis');
				$temp = "backend/cetak/laporan_hasil_kegiatan.html";
		...
		switch ($p1) {
			...
			case "pdf":
				...
				if (isset($_GET['view'])) {
					...
					$preview = str_replace(FCPATH,base_url(),$htmlcontent);
					echo $preview;
					exit;
				}
