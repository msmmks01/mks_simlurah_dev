	...
	function getdata($type = "", $balikan = "", $p1 = "", $p2 = "", $p3 = "", $p4 = "")
	{
		...
		switch ($type) {
			...
			case "laporan_penduduk":
				...
				if ($status_data != '') {
					$where .= "and A.status_data like '%$status_data%'";
				}


				$sql = "

					SELECT A.*,CONCAT(LEFT(nik,13),'xxx') AS nik2,CONCAT(LEFT(no_kk,13),'xxx') AS no_kk2,
			...
			case "laporan_rt_rw":
				...
				SELECT a.*,DATE_FORMAT(a.create_date, '%d-%m-%Y %H:%i') as tanggal_buat,c.nama_agama,b.alamat from tbl_data_rt_rw a 
				
				left join tbl_data_penduduk b on a.nik=b.nik 
				
				left join cl_agama c on b.agama=c.id
			...
			case "laporan_penilaian_rt_rw":
				...
				// 			(1) nor,
				// 			(0) AS id,
				// 			kategori_penilaian_rt_rw_id AS xid,
				// 			a.kategori,
				// 			kategori AS uraian,
				// 			a.satuan,
				// 			a.target,
				...
				// 			(0) AS nilai
				// 		FROM
				// 			tbl_penilaian_rt_rw a
				// 			LEFT JOIN cl_pilih_bulan b
				// 			ON a.bulan = b.id
				// 			LEFT JOIN tbl_data_rt_rw c
				// 			ON a.nik = c.nik
				// 			LEFT JOIN cl_kelurahan_desa d
				// 			ON a.cl_kelurahan_desa_id = d.id
				// 			AND a.cl_kecamatan_id = d.kecamatan_id
				// 		WHERE 1 = 1
				// 			AND a.cl_kelurahan_desa_id = '$desa_id'
				// 			AND a.nik = '$nik'
				...
				// 		SELECT
				// 			(2) nor,
				// 			kategori_penilaian_rt_rw_id AS id,
				// 			kategori_penilaian_rt_rw_id AS xid,
				// 			a.kategori,
				// 			a.uraian,
				// 			a.satuan,
				...
				// 			MONTH(tgl_surat),
				// 			YEAR(tgl_surat),
				// 			a.kategori_penilaian_rt_rw_id) d
				// 		ORDER BY id,
				// 		nor) s
				// 	GROUP BY kategori,
				// 		id WITH ROLLUP) s
				...
				// 	UNION ALL
				// 	SELECT *,CASE 
				// 			WHEN CAST(nilai_rata AS CHAR)<'60'  THEN ''

				// 		WHEN CAST(nilai_rata AS CHAR)>='60' AND  CAST(nilai_rata AS CHAR)<='70' THEN 'CUKUP'
				// 		WHEN CAST(nilai_rata AS CHAR)>='71' AND  CAST(nilai_rata AS CHAR)<='80' THEN 'CUKUP BAIK'
				// 		WHEN CAST(nilai_rata AS CHAR)>='80' AND  CAST(nilai_rata AS CHAR)<='90' THEN 'BAIK'
				// 		WHEN CAST(nilai_rata AS CHAR)>='91' AND  CAST(nilai_rata AS CHAR)<='100' THEN 'SANGAT BAIK'
				// 		END AS penilaian FROM ( 
				// 	SELECT
				// 		'99'nor,
				// 		IFNULL(id, 99) id,
				// 		'99'xid,
				...
				// 		''capaian
				// 		,0 AS nilai
				// 		,SUM(IFNULL(nilai, 0)) nilai_max
				// 		,CAST(FORMAT(SUM(IFNULL(nilai/25, 0)),2) AS CHAR) nilai_rata
				// 	FROM
				// 		(SELECT
				// 		*
				// 		FROM
				...
				// $sql ="SELECT (case when nor=1 then @nh:=@nh+1 end)urutHeader,a.* 
				// 	from (
				// 		select a.nor,
				// 			a.xid,a.singkatan,a.kategori,a.uraian,(case when a.nor=3 then a.satuan else b.satuan end)satuan,b.target,b.capaian,b.nilai 
				// 		from (
				// 			select id,(null)xid,(1)nor,singkatan,kategori,('')uraian,(0)satuan from tbl_kategori_penilaian_rt_rw group by singkatan
				// 			union all
				// 			select id,(id)xid,(2)nor,singkatan,kategori,uraian,(0)satuan from tbl_kategori_penilaian_rt_rw
				// 			union all 
				// 			select max(id)id,(null)xid,(3)nor,('')singkatan,('')kategori,('Maksimal Skor')uraian,(count(id)*100)satuan from tbl_kategori_penilaian_rt_rw group by singkatan
				// 		)a left join (select a.kategori_penilaian_rt_rw_id,a.nik,a.satuan,a.target,a.capaian,a.nilai 
				// 					from tbl_penilaian_rt_rw a
				// 					where a.nik='$nik' and a.bulan='$bulan' )b on b.kategori_penilaian_rt_rw_id=a.xid
				// 		,(select @nr:=0,@nh:=0)urt
				// 		order by a.id,nor)a
				// ";
				...
				$sql = " SELECT  (CASE WHEN nor = 1 THEN @nh := @nh + 1 ELSE NULL END) AS urutHeader,a.*
						...
						LEFT JOIN (
							...
							WHERE nik = '$nik' AND bulan = '$bulan'
						) b ON b.kategori_penilaian_rt_rw_id = a.xid,
						(SELECT @nh := 0) urut
						ORDER BY a.kategori, a.id, nor
						) a
				";
			...
			case "laporan_rekap_penilaian_rt_rw":
				...
				// $sql=" SELECT nor,a.nik, b.nama_lengkap, b.no_npwp, b.no_rekening, b.jab_rt_rw, a.bulan, a.kategori, a.nil_indikator, a.rekap, (rekap/jml_indikator)rata_rata, 
				// 		CASE WHEN rata_rata < '60' THEN 'Kurang' WHEN rata_rata >= '60' AND rata_rata <= '70' THEN 'Cukup' 
				// 			WHEN rata_rata >= '71' AND rata_rata <= '80' THEN 'Cukup Baik' 
				// 			WHEN rata_rata >= '81' AND rata_rata <= '90' THEN 'Baik' 
				// 			WHEN rata_rata >= '91' THEN 'Sangat Baik' END AS standar, 
				// 		CASE WHEN rata_rata < '60' THEN '0' WHEN rata_rata >= '60' AND rata_rata <= '70' THEN '300000' 
				// 			WHEN rata_rata >= '71' AND rata_rata <= '80' THEN '600000' 
				// 			WHEN rata_rata >= '81' AND rata_rata <= '90' THEN '900000' 
				// 			WHEN rata_rata >= '91' THEN '1200000' END AS insentif 
				// 	FROM ( SELECT nor,nik, nama_lengkap, jab_rt_rw, bulan, 
				// 		GROUP_CONCAT(singkatan ORDER BY CASE singkatan WHEN 'PBB' THEN 1 WHEN 'Usaha Unggulan' THEN 2 WHEN 'Urban Farming' THEN 3 WHEN 'Iuran Sampah' THEN 4 WHEN 'Penduduk Non Permanen' THEN 5 WHEN 'Kerawanan Bencana' THEN 6 WHEN 'Kerawanan Sosial' THEN 7 WHEN 'Deteksi Ketertiban dan Keamanan' THEN 8 WHEN 'Buku Administrasi' THEN 9 ELSE 99 END ) AS kategori, 
				// 		GROUP_CONCAT(nilai ORDER BY CASE singkatan WHEN 'PBB' THEN 1 WHEN 'Usaha Unggulan' THEN 2 WHEN 'Urban Farming' THEN 3 WHEN 'Iuran Sampah' THEN 4 WHEN 'Penduduk Non Permanen' THEN 5 WHEN 'Kerawanan Bencana' THEN 6 WHEN 'Kerawanan Sosial' THEN 7 WHEN 'Deteksi Ketertiban dan Keamanan' THEN 8 WHEN 'Buku Administrasi' THEN 9 ELSE 99 END ) AS nil_indikator, SUM(nilai) AS rekap, 
				// 		SUM(jml_sub_indikator)jml_indikator, (SUM(nilai) / SUM(jml_sub_indikator)) AS rata_rata FROM ( 
				// 			SELECT (1)nor,a.nik, b.nama_lengkap, b.jab_rt_rw, c.singkatan, a.bulan, COUNT(a.kategori) AS jml_sub, COUNT(a.uraian) AS jml_sub_indikator, SUM(a.nilai) AS nilai 
				// 			FROM tbl_penilaian_rt_rw a LEFT JOIN tbl_data_rt_rw b ON a.nik = b.nik 
				// 			LEFT JOIN tbl_kategori_penilaian_rt_rw c ON a.kategori_penilaian_rt_rw_id = c.id 
				// 			WHERE 1=1 AND a.cl_provinsi_id = '73' AND a.cl_kab_kota_id = '7371' AND a.cl_kecamatan_id = '7371110' 
				// 			AND a.cl_kelurahan_desa_id = '7371110001' AND a.bulan LIKE '%6%' 
				// 			GROUP BY a.kategori, a.nik, b.nama_lengkap, a.bulan 
				// 			UNION ALL 
				// 			SELECT (2)nor,b.nik, b.nama_lengkap, b.jab_rt_rw, ('')singkatan, ('')bulan, (0) AS jml_sub, (0) AS jml_sub_indikator, (0) AS nilai 
				// 			FROM tbl_data_rt_rw b 
				// 			WHERE 1=1 AND b.cl_provinsi_id = '73' AND b.cl_kab_kota_id = '7371' AND b.cl_kecamatan_id = '7371110' AND b.cl_kelurahan_desa_id = '7371110001' 
				// 			AND b.nik NOT IN (SELECT a.nik FROM tbl_penilaian_rt_rw a WHERE a.bulan='6')
				// 			GROUP BY b.nik, b.nama_lengkap
				// 		) a GROUP BY nik, nama_lengkap, jab_rt_rw ) a LEFT JOIN tbl_data_rt_rw b ON a.nik = b.nik AND cl_provinsi_id = '73' AND cl_kab_kota_id = '7371' AND cl_kecamatan_id = '7371110' AND cl_kelurahan_desa_id = '7371110001' ORDER BY nor,a.nik DESC;
				// ";

				$sql = "SELECT 
							nor, a.nik, b.nama_lengkap, b.no_npwp, b.no_rekening, b.jab_rt_rw, b.rw, b.rt,
							a.bulan, a.kategori, a.nil_indikator, a.rekap, (rekap / jml_indikator) AS rata_rata,
							CASE 
								WHEN (rekap / jml_indikator) < 60 THEN 'Kurang'
								WHEN (rekap / jml_indikator) BETWEEN 60 AND 70 THEN 'Cukup'
								WHEN (rekap / jml_indikator) BETWEEN 71 AND 80 THEN 'Cukup Baik'
								WHEN (rekap / jml_indikator) BETWEEN 81 AND 90 THEN 'Baik'
								WHEN (rekap / jml_indikator) > 90 THEN 'Sangat Baik'
							END AS standar,
							CASE 
								WHEN (rekap / jml_indikator) < 60 THEN '0'
								WHEN (rekap / jml_indikator) BETWEEN 60 AND 70 THEN '300000'
								WHEN (rekap / jml_indikator) BETWEEN 71 AND 80 THEN '600000'
								WHEN (rekap / jml_indikator) BETWEEN 81 AND 90 THEN '900000'
								WHEN (rekap / jml_indikator) > 90 THEN '1200000'
							END AS insentif
							FROM (
							SELECT 
								nor, nik, nama_lengkap, jab_rt_rw, bulan,
								GROUP_CONCAT(singkatan ORDER BY urut) AS kategori,
								GROUP_CONCAT(nilai ORDER BY urut) AS nil_indikator,
								SUM(nilai) AS rekap,
								SUM(jml_sub_indikator) AS jml_indikator
							FROM (
								SELECT 
								1 AS nor,
								a.nik, b.nama_lengkap, b.jab_rt_rw, a.bulan,
								singkatan,  
								99 AS urut,       
								...
								SUM(a.nilai) AS nilai
								FROM tbl_penilaian_rt_rw a
								LEFT JOIN tbl_data_rt_rw b ON a.nik = b.nik
								LEFT JOIN tbl_kategori_penilaian_rt_rw c ON a.kategori_penilaian_rt_rw_id=c.id
								WHERE a.cl_provinsi_id = '" . $this->auth['cl_provinsi_id'] . "' 
								AND a.cl_kab_kota_id = '" . $this->auth['cl_kab_kota_id'] . "'
								AND a.cl_kecamatan_id = '" . $this->auth['cl_kecamatan_id'] . "'
								AND a.cl_kelurahan_desa_id = '" . $this->auth['cl_kelurahan_desa_id'] . "'
								AND a.bulan LIKE '%$bulan%'
								GROUP BY a.kategori, a.nik, b.nama_lengkap, a.bulan
								...
								SELECT 
								2 AS nor,
								b.nik, b.nama_lengkap, b.jab_rt_rw,
								'' AS bulan,
								'' AS singkatan,
								99 AS urut,
								0 AS jml_sub,
								...
								0 AS nilai
								FROM tbl_data_rt_rw b
								WHERE b.cl_provinsi_id = '" . $this->auth['cl_provinsi_id'] . "' 
								AND b.cl_kab_kota_id = '" . $this->auth['cl_kab_kota_id'] . "'
								AND b.cl_kecamatan_id = '" . $this->auth['cl_kecamatan_id'] . "'
								AND b.cl_kelurahan_desa_id = '" . $this->auth['cl_kelurahan_desa_id'] . "'
								AND b.nik NOT IN (
									SELECT nik FROM tbl_penilaian_rt_rw WHERE bulan = '$bulan'
								)
								GROUP BY b.nik, b.nama_lengkap
							) sub
							GROUP BY nik, nama_lengkap, jab_rt_rw
							) a
							LEFT JOIN tbl_data_rt_rw b 
							ON a.nik = b.nik 
							AND b.cl_provinsi_id = '" . $this->auth['cl_provinsi_id'] . "'
							AND b.cl_kab_kota_id = '" . $this->auth['cl_kab_kota_id'] . "'
							AND b.cl_kecamatan_id = '" . $this->auth['cl_kecamatan_id'] . "'
							AND b.cl_kelurahan_desa_id = '" . $this->auth['cl_kelurahan_desa_id'] . "'
							ORDER BY b.rw, b.rt, nor, a.nik;
			...
			case "data_indikator_skm":
				...
				$sql = "
				SELECT a.*,b.nama AS kecamatan FROM tbl_indikator_skm a
			...
			case "data_penilaian_skm":
				...
				$sql = "
				SELECT c.*,ROUND((c.nilai / IFNULL(c.jumlah_indikator, 0)),2) AS rata_rata FROM(
			...
			case "cetak_surat":
				...
				$sql = "
					...
					SELECT A.*, B.nama_pekerjaan, C.nama_agama as agama,E.nip,E.masa_berlaku,E.status_usaha,F.nama,F.jabatan,F.pangkat,
						...
						CONCAT(
						  ...
						  E.data_surat,G.nm_golongan,
						  E.tgl_surat

					FROM (
						SELECT *,('0')wn,'' AS nama_wn,'' AS keperluan_passport,'' AS tgl_kel_passport,'' AS tgl_akhir_passport,'' AS alamat_asal,'' AS jenis_passport FROM tbl_data_penduduk 
						UNION
					...
					RIGHT JOIN tbl_data_surat E ON A.id = E.tbl_data_penduduk_id

  					LEFT JOIN tbl_data_penandatanganan F ON E.nip = F.nip and F.cl_kelurahan_desa_id = E.cl_kelurahan_desa_id

					LEFT JOIN cl_golongan G on F.pangkat = G.pangkat
			...
			case "data_rt_rw":
				...
				$sql = "SELECT a.*,c.nama_agama,d.nama,b.alamat as nama_keluahan_desa from tbl_data_rt_rw a
			...
			case "penilaian_rt_rw":
				...
				$sql = "
					...
					SELECT a.penilaian_id as id,a.nama_lengkap,a.tgl_surat,a.kategori_penilaian_rt_rw_id,a.kategori,a.uraian,a.satuan,a.target,a.capaian,
					ROUND(SUM(a.nilai)/COUNT(a.id),2) AS nilai,
					CASE 
						WHEN ROUND(SUM(a.nilai)/COUNT(a.id),2) BETWEEN 60 AND 70 THEN 'Cukup'
						WHEN ROUND(SUM(a.nilai)/COUNT(a.id),2) BETWEEN 71 AND 80 THEN 'Cukup Baik'
						WHEN ROUND(SUM(a.nilai)/COUNT(a.id),2) BETWEEN 81 AND 90 THEN 'Baik'
						WHEN ROUND(SUM(a.nilai)/COUNT(a.id),2) BETWEEN 91 AND 100 THEN 'Sangat Baik'
						ELSE 'Tidak Valid'
					END AS standar_nilai,
					CASE 
						WHEN ROUND(SUM(a.nilai)/COUNT(a.id),2) BETWEEN 60 AND 70 THEN 'Rp. 300.000,00'
						WHEN ROUND(SUM(a.nilai)/COUNT(a.id),2) BETWEEN 71 AND 80 THEN 'Rp. 600.000,00'
						WHEN ROUND(SUM(a.nilai)/COUNT(a.id),2) BETWEEN 81 AND 90 THEN 'Rp. 900.000,00'
						WHEN ROUND(SUM(a.nilai)/COUNT(a.id),2) BETWEEN 91 AND 100 THEN 'Rp. 1.200.000,00'
						ELSE 0
					END AS usulan_insentif,
					a.create_date,a.create_by,c.nik,c.nama_lengkap,a.bulan,b.nama_bulan,d.nama AS nama_keluahan_desa,
					...
					left join tbl_data_rt_rw c ON a.nik = c.nik
					
					left join cl_kelurahan_desa d on a.cl_kelurahan_desa_id=d.id and a.cl_kecamatan_id=d.kecamatan_id

					$where

					GROUP BY c.nama_lengkap, MONTH(tgl_surat), YEAR(tgl_surat)
			...
			case "rekap_penilaian_kelrtrw":
				...
				$sql = " SELECT a.id,a.nama_lengkap,a.tgl_surat,a.kategori_penilaian_rt_rw_id,a.kategori,a.uraian,a.satuan,a.target,a.capaian,
					ROUND(SUM(a.nilai)/COUNT(a.id),2) AS nilai,
					a.create_date,a.create_by,c.nik,c.nama_lengkap,b.nama_bulan,d.nama AS nama_keluahan_desa,
		...
		switch ($table) {
			...
			case "data_rt_rw":
				...
				if ($sts_crud == "add" || $sts_crud == "edit") {
					...
					$data['nama_lengkap'] = $nama_penduduk['nama_lengkap'];
				}
			...
			case "penilaian_rt_rw":
				...
				if ($sts_crud == "add") {
					$nama_penduduk = $this->db->get_where('tbl_data_rt_rw', array('nik' => $data['nik']))->row_array();
					...
					for ($i = 0; $i < count($kategori_penilaian_rt_rw_id); $i++) {
						$data[] = [
							...
							'cl_kecamatan_id' => $this->auth['cl_kecamatan_id'],
							'cl_kelurahan_desa_id' => $this->auth['cl_kelurahan_desa_id'],
							'id_pegawai' => $nama_penduduk['id'],
							'nik' => $nama_penduduk['nik'],
							'nama_lengkap' => $nama_penduduk['nama_lengkap'],
							'tgl_surat' => $tgl_surat,
				...
				if ($sts_crud == "edit") {
					...
					$this->db->delete('tbl_penilaian_rt_rw', array('nik' => $nama_penduduk['nik'], 'bulan' => $bulan));
					...
					for ($i = 0; $i < count($kategori_penilaian_rt_rw_id); $i++) {
						$data_to_insert[] = [
							...
							'nilai' => $nilai[$i],
							'update_date' => date('Y-m-d H:i:s'),
							'update_by' => $this->auth['nama_lengkap'],
							'file' => @$file
						];
					}
				}
		...
		switch ($sts_crud) {
			...
			case "add":
				if ($table == "tbl_data_penduduk") {
					...
					$cek = $this->db->select('nip')->get_where('tbl_data_penandatanganan', array('nip' => $nip, 'cl_kelurahan_desa_id' => $kec));
					if ($cek->num_rows() > 0) {
						return '2';
					} else {
