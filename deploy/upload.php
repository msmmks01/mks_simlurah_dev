...
if (!isset($_GET['dir'])) {
	...
	if ($_GET['dir'] == '') {
		echo ("<script type=\"text/javascript\">alert('Tidak ada file untuk di upload'); location.href='" . url() . "';</script>");
		exit();
	}
}
...
try {
	...
	foreach ($Sheets as $Index => $Name) {
		...
		foreach ($Spreadsheet as $Key => $Row) {
			echo $Key . ': ' . PHP_EOL;
			if ($Row) {
				/*if (ftp_put($ftp_conn, $Row[0].$_GET['dir'], $file, FTP_ASCII)){
						echo "<span style=\"color:green\">".date('d-m-Y H:i:s')." - Successfully uploaded $file to $Row[0]</span><br>";
					}else{
						echo "<span style=\"color:red\">".date('d-m-Y H:i:s')." - Error uploading uploaded $file to $Row[0]</span><br>";
					}*/
				$from = $file;
				$to = $Row[0] . $_GET['dir'];
				if (!copy($from, $to)) {
